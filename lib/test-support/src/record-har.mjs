...
const run = (command, commandArgs, env = process.env) =>
  new Promise(resolve => {
    const shellArgs = process.platform === 'win32' ? commandArgs.map(quoteArg) : commandArgs;
    console.log('running', command, shellArgs.join(' '));
...
const main = async () => {
  const testExitCode = await run('yarn', ['test', '--max-failures=0', ...args], {
    ...process.env,
    UPDATE_HAR: '1',
  });
