# ACUM work importer

[![install](https://img.shields.io/badge/install-latest-3c9a40?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDwvc3ZnPg%3D%3D&labelColor=lightyellow)](../../scripts/acum-work-import/acum-work-import.user.js?raw=1)
[![source](https://img.shields.io/badge/source-lightyellow?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%%3D)](app.ts)

This scripts allows you to import works from the database of the Israeli rights management society [ACUM](https://acum.org.il/en).

![ACUM importer](acum-work-import.png?raw=1)

To import works for a release:

1. open the release relationship editor
2. select the recordings whose works you want to import
3. find the album ID, this will be in the end of the album URL in ACUM, e.g. https://nocs.acum.org.il/acumsitesearchdb/album?albumid=011820
4. insert the ACUM album ID in the input box
5. click the import button.

The script will then create new works, link any writer and arranger it can find in the database, and link the works to the selected recordings.

Since ACUM database treats every medium as a separate album all selected recordings should be under a single medium.

## Release Notes

See [CHANGELOG.md](CHANGELOG.md).
