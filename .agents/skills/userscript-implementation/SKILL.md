...
- Record and refresh HAR files with `--project=chromium-har` and commit them under `tests/fixtures/har/`.
...
7. Refresh HARs when test data changes (for example, `yarn workspace @dvirtz/<script-name> test --project=chromium-har --reporter=line`).
