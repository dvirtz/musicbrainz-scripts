...
function submitEvent() {
    ...
    if (tour) {
        ...
        searchParams.append("edit-event.name", `${tour}: ${sfmPageAttributes.venue.city}`);
    } else {
        // use "Artist at Venue" style 
        searchParams.append("edit-event.name", `${sfmPageAttributes.artist.name} at ${sfmPageAttributes.venue.name}`);
    }
    ...
    // setlist
    const artistName = sfmPageAttributes.artist.name;
    const artistMBID = sfmPageAttributes.artist.mbid;
    const setlist = Array.from(document.querySelectorAll('.setlistParts').values()).flatMap(part => [...setlistEntry(part)]).join('\n');
    ...
    searchParams.append("edit-event.edit_note", `Imported from ${document.location.href} using ${GM.info.script.name} version ${GM.info.script.version} from ${GM.info.script.homepageURL}.`);
...
const parent = document.querySelector('.user-fragment');
...
function* setlistEntry(setlistPart) {
    if (setlistPart.classList.contains('tape') || setlistPart.classList.contains('song')) {
        ...
        if (infoPart) {
            yield* infoPart.textContent.split('\n').filter(line => line.trim().length > 0).map(line => info(line));
        }
