...
var executeFetch = function(requestObj, method, url, body, attempt) {
attempt = attempt || 0;
if (requestObj.precondition()) {
requestObj.log(method, url);
Wicket.Ajax.invokePreCallHandlers();
var headers = {
"Wicket-Ajax": "true",
"Accept": "text/xml"
...
requestBody = (typeof (body) === "function") ? body() : body;
}
if (Wicket.Focus && Wicket.Focus.lastFocusId) {
headers["Wicket-FocusedElementId"] = Wicket.Focus.lastFocusId;
}
var fetcher = (typeof AwsWafIntegration === 'object') ? AwsWafIntegration.fetch : fetch;
fetcher(url, {
method: method,
headers: headers,
body: requestBody
}).then(function(response) {
var action = response.headers.get('x-amzn-waf-action');

 if (action && attempt < 3 && typeof AwsWafIntegration === 'object') {
var retry = function() {
executeFetch(requestObj, method, url, body, attempt + 1);
};
if (action === 'challenge') {

 retry();
} else if (action === 'captcha') {
if (typeof AwsWafCaptcha !== 'object') {
Wicket.Log.error("WAF Captcha not available to handle response");
requestObj.failure();
} else {
var $modal = $('<div class="modal awsCaptchaModal"><div class="modal-dialog" role="document"><div class="modal-content">' +
'<div class="modal-header"><h4 class="modal-title">Are we human ...</h4></div>' +
'<div class="modal-body"><div id="awsCaptcha">... or are we robot?</div></div>' +
'</div></div></div>'
).appendTo("body").modal({
...
AwsWafCaptcha.renderCaptcha(document.querySelector('#awsCaptcha'), {
apiKey: window.awsWebAclCaptchaApiKey,
skipTitle: true,
dynamicWidth: true,
onSuccess: function(newToken) {
$modal.modal('hide').remove();
retry();
},
onError: function(error) {
Wicket.Log.error("WAF Captcha Error: " + error);
$modal.remove();
requestObj.failure();
}
});
...
Wicket.Log.error("unexpected WAF action: " + action);
}
} else {

 response.text().then(function(text) {
requestObj.transport = {
readyState: 4,
...
requestObj.stateChangeCallback();
});
}
}).catch(function(error) {
Wicket.Log.error("Wicket Fetch Error: " + error);
requestObj.failure();
});
return true;
} else {
Wicket.Log.info("Ajax " + method + " stopped by precondition: " + requestObj.url);
requestObj.done();
return true;
}
};
Wicket.Ajax.Request.prototype.doGet = function() {
return executeFetch(this, "GET", this.createUrl(), null);
};
Wicket.Ajax.Request.prototype.doPost = function(body) {
return executeFetch(this, "POST", this.createUrl(), body);
};
})();
