...
function getAvatarLetterColor(letter) {
const up = (typeof letter === 'string' ? letter.charAt(0) : '').toUpperCase();
if (!/^[A-Z]$/.test(up)) return '#CCCCCC';

 const index = up.charCodeAt(0) - 65; 
 const hue = Math.round((index * 360) / 26); 
 const pastelS = 60;
const pastelL = 80;
return hslToHex(hue, pastelS, pastelL);
