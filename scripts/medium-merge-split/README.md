# Medium Merge Split

[![install][badge-install]](https://github.com/dvirtz/musicbrainz-scripts/releases/latest/download/medium-merge-split.user.js) [![beta][badge-beta]](https://github.com/dvirtz/musicbrainz-scripts/releases/download/beta-latest/medium-merge-split.user.js) [![source][badge-source]](src/index.ts)

Merge and split mediums in the MusicBrainz release editor

## Usage

The script adds controls to the **Tracklist** tab of the release editor.

Open the `dvirtz MusicBrainz scripts` toolbox and click **Show** to toggle the controls on and off.

![toolbox button](assets/toolbox-button.png?raw=1)

### Merge mediums

Next to each medium's *move medium up* / *move medium down* buttons, two buttons are added:

- **Merge with next medium** — appends the following medium's tracks to this one and removes it.
- **Merge with previous medium** — appends this medium's tracks to the previous one and removes this medium.

![merge buttons](assets/merge-buttons.png?raw=1)

The buttons are disabled when there is no adjacent medium, when either medium still has to be
expanded so its tracklist is loaded, or when either medium has a disc ID.

### Split a medium

Next to each track's *move track up* / *move track down* buttons, a **Split medium before this track**
button is added. It moves that track and every track after it to a new medium inserted right after the
current one.

![split button](assets/split-button.png?raw=1)

The button is hidden on the first track of a medium and disabled for mediums with a disc ID.

Both actions move the existing track objects, so recordings, artist credits and track lengths are kept,
and track numbers are renumbered on the affected mediums.

## Release Notes

See [CHANGELOG.md](CHANGELOG.md).

[badge-install]: https://img.shields.io/badge/install-latest-3c9a40?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDwvc3ZnPg%3D%3D&labelColor=lightyellow
[badge-beta]: https://img.shields.io/badge/install-beta-d83a3a?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDxnPiA8L2c%2BIDwvc3ZnPg%3D%3D&labelColor=lightyellow
[badge-source]: https://img.shields.io/badge/source-lightyellow?style=for-the-badge&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%%3D
