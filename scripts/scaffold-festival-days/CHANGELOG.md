## @dvirtz/scaffold-festival-days-v1.1.1 (2026-04-03)

* fix(scaffold-festival-days): avoid creating empty day events ([1b24f73](https://github.com/dvirtz/musicbrainz-scripts/commit/1b24f73))

## @dvirtz/scaffold-festival-days-v1.1.0 (2026-03-31)

* feat(event-seeder): clone an event ([6f72397](https://github.com/dvirtz/musicbrainz-scripts/commit/6f72397))
* feat(scaffold-festival-days): better matching ([9d7e3d5](https://github.com/dvirtz/musicbrainz-scripts/commit/9d7e3d5))
* test(scaffold-festival-days): verify page reload ([b6b0286](https://github.com/dvirtz/musicbrainz-scripts/commit/b6b0286))

## @dvirtz/scaffold-festival-days-v1.0.0 (2026-03-22)

* feat: scaffold festival days userscript ([1eed194](https://github.com/dvirtz/musicbrainz-scripts/commit/1eed194))
* feat(add-sub-event): add sub-event userscript and extract shared event helpers ([29cab50](https://github.com/dvirtz/musicbrainz-scripts/commit/29cab50))

# Changelog

All notable changes to `@dvirtz/scaffold-festival-days` will be documented in this file.

## 1.0.0 (INITIAL)

- Initial scaffold
