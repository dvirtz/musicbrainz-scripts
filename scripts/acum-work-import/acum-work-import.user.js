...
function releaseEditorTools() {
  ...
  const toolbox = (() => {
    var _el$ = _tmpl$$e();
    _el$.style.setProperty("padding", "8px");
    _el$.style.setProperty("border", "5px dotted rgb(171, 171, 109)");
    _el$.style.setProperty("margin", "0px -6px 6px");
    return _el$;
  })();
...
const tryFetchJSON$1 = (url, options) => tryFetch(fetchJSON$1)(url, options);
async function fetchJSON$1(url, options) {
  const response = await fetchResponse$1(url, Object.assign(Object.assign({
    headers: {
...
async function fetchText(url, options) {
  const response = await fetchResponse$1(url, options);
  return await response.text();
}
async function fetchResponse$1(url, options) {
...
async function getWorkVersions(workId) {
  const result = await tryFetchJSON$1(`https://nocs.acum.org.il/acumsitesearchdb/getworkinfo?workId=${workId}`);
  if (result) {
...
async function getAlbumInfo(albumId) {
  const result = await tryFetchJSON$1(albumApiUrl(albumId));
  if (result) {
    if (result.errorCode == 0) {
...
const intervalQueues = new Map();
async function rateLimited(interval, fn) {
  if (!intervalQueues.has(interval)) {
    intervalQueues.set(interval, new PQueue({
      interval,
      intervalCap: 1
    }));
  }
  return await intervalQueues.get(interval).add(fn, {
    throwOnTimeout: true
  });
}

async function tryFetchJSON(url, options) {
  return await rateLimited(RATE_LIMIT_INTERVAL, () => fetchJSON$1(url, options));
}
async function fetchJSON(url, options) {
  return await rateLimited(RATE_LIMIT_INTERVAL, () => fetchJSON$1(url, options));
}
async function fetchResponse(url, options) {
  return await rateLimited(RATE_LIMIT_INTERVAL, () => fetchResponse$1(url, options));
}
